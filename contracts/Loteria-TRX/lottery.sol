...
library RandomNumber{
    ...
    function randMod(uint256 _modulus, uint256 _moreRandom,uint256 _randNonce) internal view returns(uint256){
       return uint256(keccak256(abi.encodePacked(block.timestamp, msg.sender, _moreRandom,_randNonce))) % _modulus;
...
contract Lottery {
    ...
    function allValueNFTs(address _user)public view returns(uint256 tron){
        ...
        for (uint256 index = 0; index < array; index++) {
            tron = tron.add(vaul[TRC721_Contract.tokenOfOwnerByIndex(_user,index)]);
        }
    }
    ...
    function changeBeneficioTeam(uint256 _beneficio, uint256 _precicion) public {
        onlyOwner();
