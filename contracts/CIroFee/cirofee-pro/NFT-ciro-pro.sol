...
contract TRC721 is TRC165, MinterRole {
    using SafeMath for uint256;
    ...
    mapping(uint256 => uint256) public typeToken;
    uint256[] public feeList = [1500, 3000, 5000];
    uint256 public defaultPrecision = 10000;

    // Token name
    string private _name = "Ciro Fee Pro";

    // Token symbol
    string private _symbol = "CFP";

    // Base URI
    string private _baseURI = "https://nft-metadata.cirotrx.com/v1/nft/type/";
    ...
    function updateDefaultPrecision(uint256 _precision) public onlyMinter {
        defaultPrecision = _precision;
    }
    ...
    function mintNftCiro(
        address to,
        uint256 _type
    ) public onlyMinter returns (bool) {
        require(feeList.length > 0, "The type does not exist");
        uint256 tokenId = totalSupply();
        string memory token_URI = string(
            abi.encodePacked(_baseURI, uint2str(_type))
        );
        typeToken[tokenId] = _type;
        _mint(to, tokenId);
        _setTokenURI(tokenId, token_URI);
        return true;
    }
    ...
    function getDiscount(uint256 _tokenId) public view returns (uint256) {
        return feeList[typeToken[_tokenId]];
    }
    ...
    function getDiscountUser(
        ...
        public
        view
        returns (bool discount, uint256 fee, uint256 precision, uint256 tipo)
    {
        uint256[] memory arrayNFT = _tokensOfOwner(_user);
        if (arrayNFT.length > 0) {
            discount = true;
            uint256 _tokenId = arrayNFT[0];
            for (uint256 index = 1; index < arrayNFT.length; index++) {
                if (
                    feeList[typeToken[arrayNFT[index]]] >
                    feeList[typeToken[_tokenId]]
                ) {
                    _tokenId = arrayNFT[index];
                }
            }
            fee = feeList[typeToken[_tokenId]];
            tipo = typeToken[_tokenId];
        }

        precision = defaultPrecision;
    }

    function updateFeelist(uint256[] calldata _feeList) public {
        feeList = _feeList;
    }
