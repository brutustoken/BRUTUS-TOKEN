...
contract SimpleSwapV4 is Storage_1 {
    ...
    function sell_token_2_to(
        ...
        uint256 pago = (_value_t2 * RATE()) /
            (10 ** Token_2_Contract.decimals());

        require(balance_token_1() >= pago);
        ...
        if (
            ...
            pago = (pago * (precision - descuentoRapido)) / (precision);
            _value_t2 =
                (_value_t2 * (precision - descuentoRapido)) /
                (precision);
        }
