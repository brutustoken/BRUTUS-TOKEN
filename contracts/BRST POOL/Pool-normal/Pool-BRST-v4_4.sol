...
contract PoolBRST_v4_4 {
    ...
    // almacenamiento adicional debajo de esta linea
    ...
    function donate() public payable {
        if (msg.value == 0) revert("No TRX sent");
        _WALLET_SR_BALANCE = _WALLET_SR_BALANCE + msg.value;
    }

    function donate(uint256 _value) public {
        if (_value == 0) revert("No value token sent");
        BRST_Contract.transferFrom(msg.sender, address(this), _value);
    ...
    function staking() public payable returns (uint256) {
        ...
        _value = (_value * (10 ** BRST_Contract.decimals())) / RATE();
        _WALLET_SR_BALANCE = _WALLET_SR_BALANCE + msg.value;
    ...
    function retirar(uint256 _id) public returns (bool exitoso) {
        ...
        if (msg.sender != owner()) {
            if (block.timestamp < onPeticion.tiempo + TIEMPO)
                revert("no time to claim");
        }

        uint256 pago = (onPeticion.brst * onPeticion.precio) /
            (10 ** BRST_Contract.decimals());

        require(address(this).balance >= pago);

        if (misSolicitudes[onPeticion.wallet].length >= 2) {
            (bool success, uint256 i) = misSolicitudes[onPeticion.wallet]
                .findIndexOf(_id);

            if (success) {
                misSolicitudes[onPeticion.wallet][i] = misSolicitudes[
                    onPeticion.wallet
                ][misSolicitudes[onPeticion.wallet].length - 1];
            }
        }
        ...
        if (misSolicitudes[onPeticion.wallet].length > 0) {
            misSolicitudes[onPeticion.wallet].pop();
        }

        BRST_Contract.redeem(
            (pago * (10 ** BRST_Contract.decimals())) / (RATE())
        );
    ...
    function cancelarOrden(uint256 _id) public {
        onlyOwner();
        require(index > _id);
        require(!completada[_id]);
        Peticion memory onPeticion = peticiones[_id];
    ...
    function asignarPerdida(uint256 _value) public {
        onlyOwner();
        _WALLET_SR_BALANCE = _WALLET_SR_BALANCE - _value;
    }
    ...
    function sumarDisponible(uint256 _value) public {
        onlyOwner();
        totalDisponible = totalDisponible + _value;
    }
    ...
    function setTronSolicitado(uint256 _value) public {
        onlyOwner();
        TRON_SOLICITADO = _value;
    }
    ...
    function redimTRX(uint256 _value, address _to) public returns (uint256) {
        onlyOwner();
        if (address(this).balance < _value) revert();
