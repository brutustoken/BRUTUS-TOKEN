...
const Alert = React.lazy(() => import("./components/Alert.jsx"));
const Home = React.lazy(() => import("./pages/Home.jsx"));
const Brut = React.lazy(() => import("./pages/BRUT.jsx"));
const Brst = React.lazy(() => import("./pages/BRST-Proxy.jsx"));
const Nft = React.lazy(() => import("./pages/BRGY.jsx"));
const LOTERIA = React.lazy(() => import("./pages/BRLT.jsx"));
...
const App = ({ i18n, t }) => {
  ...
  const route = useCallback(() => {
    ...
    if (url.indexOf("/?") >= 0) {
      ...
      if (url.indexOf("&") >= 0) url = url.split("&")[0];
      if (url.indexOf("=") >= 0) url = url.split("=")[0];
      if (url === window.location.origin + "/" || url === "utum_source")
        url = "";
      url = `/#/${url}`;
    ...
    if (loginElement) {
      if (accountAddress !== addressDefault && tronlink.loggedIn) {
        ...
        loginElement.innerHTML =
          '<span id="conectTL" class="btn btn-primary" style="cursor:pointer" title="Connect your wallet to perform transactions">Connect Wallet</span> <img src="images/TronLinkLogo.png" height="40px" alt="TronLink logo" />';
    ...
    async (accountAddress) => {
      ...
      try {
        ...
        if (contrato.BRST_TRX === null && config.SC2 !== "") {
          web3Contracts = await utils.getTronweb(accountAddress);
          contrato.BRST_TRX = web3Contracts.contract(
            abi_POOL_BRST_TRX,
            config.SC2,
          );
        }
        ...
        if (contrato.BRST_TRX_Proxy === null && config.ProxySC2 !== "") {
          ...
          web3Contracts = await utils.getTronweb(accountAddress);
          contrato.BRST_TRX_Proxy = web3Contracts.contract(
            abi_POOLBRST,
            config.ProxySC2,
          );
        }
        ...
        if (contrato.BRST_TRX_Proxy_fast === null && config.ProxySC3 !== "") {
          web3Contracts = await utils.getTronweb(accountAddress);
          contrato.Proxy_fast = web3Contracts.contract(
            abi_PROXY,
            config.ProxySC3,
          );

          web3Contracts = await utils.getTronweb(accountAddress);
          contrato.BRST_TRX_Proxy_fast = web3Contracts.contract(
            abi_SimpleSwap,
            config.ProxySC3,
          );
        }
          ...
          <div className="row">
            <div className="col-xl-12">
              <div className="card">
                <div
                  className="row"
                  style={{ padding: "3em", decoration: "none" }}
                >
                  <div className="col-sm-8">
                    <h1>
                      {t("preLoad", { returnObjects: true })?.[0] || "Loading"}
                      {imgLoading}
                    </h1>
                    <p>
                      {t("preLoad", { returnObjects: true })?.[1] ||
                    ...
                    {tronlink.viewer && (
                      ...
                      >
                        Loading in viewer mode - Connect your wallet to perform
                        transactions
                      </p>
                    )}
                  </div>
  ...
  return (
    <>
      <Helmet>
        <title>
          {ruta.length > 1 ? ruta.toUpperCase() + " | " : ""}Brutus.Finance
        </title>
        <meta
          property="og:title"
          content={
            (ruta.length > 1 ? ruta.toUpperCase() + " | " : "") +
            "Brutus.Finance"
          }
        />
        <meta
          property="og:description"
          content="Alquila energia, haz staking de trx y obten los mejores rendimientos del mercado"
        />
        <meta
          property="og:image"
          content={"/images/og/brutus-" + ruta + ".jpg"}
        />
