...
class Staking extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      dias: t("loading") + "...",
      days: [
        { days: 30, amount: 0, time: 0, APY: 0 },
        { days: 90, amount: 0, time: 0, APY: 0 },
        { days: 180, amount: 0, time: 0, APY: 0 },
        { days: 360, amount: 0, time: 0, APY: 0 },
      ],
      varBrut: 0,
      precioBrst: 0,
      varBrst: 0,
  ...
  async estado() {
    ...
    let precio = utils.normalizarNumero(
      await contrato.BRST_TRX_Proxy.RATE().call(),
    );
    ...
    contrato.BRST_TRX_Proxy.TRON_BALANCE()
      .call()
      .then((enBrutus) => {
        this.setState({ enBrutus: utils.normalizarNumero(enBrutus) });
      });

    contrato.BRST_TRX_Proxy.TRON_PAY_BALANCE()
      .call()
      .then((enPool) => {
        this.setState({ enPool: utils.normalizarNumero(enPool) });
      });

    contrato.BRST_TRX_Proxy.TRON_SOLICITADO()
      .call()
      .then((solicitado) => {
        this.setState({ solicitado: utils.normalizarNumero(solicitado) });
      });

    contrato.BRST.totalSupply()
      .call()
      .then((tokensEmitidos) => {
        this.setState({
          tokensEmitidos: utils.normalizarNumero(tokensEmitidos),
        });
      });

    let misBRST = await contrato.BRST.balanceOf(accountAddress)
      .call()
      .then((balanceBRST) => {
    ...
    contrato.USDT.balanceOf(accountAddress)
      .call()
      .then((balanceUSDT) => {
        this.setState({ balanceUSDT: utils.normalizarNumero(balanceUSDT) });
      });

    contrato.USDD.balanceOf(accountAddress)
      .call()
      .then((balanceUSDD) => {
    ...
    let balance = await tronWeb.trx
      .getUnconfirmedBalance(accountAddress)
      .catch(async (e) => {
    ...
    let penalty =
      (parseInt(await contrato.BRST_TRX_Proxy_fast.descuentoRapido().call()) /
        parseInt(await contrato.BRST_TRX_Proxy_fast.precision().call())) *
      100;
    ...
    fetch(
      ...
      .then((consulta) => {
        if (consulta.length > 0) {
          let promE7to1day = consulta.reduce(
            (acc, item) => acc + item.value,
            0,
          );
          promE7to1day = new BigNumber(
            promE7to1day / consulta.length,
          ).toNumber();
          ...
          if (consulta.length >= 2) {
            valorInicial = consulta[consulta.length - 1].value; // Primer valor del rango
        ...
        if (consultaData.length > 0) {
          ...
          for (let index = 0; index < dias.length; index++) {
            earnings.push({
              dias: dias[index],
              trx:
                misBRST * consultaData[0].value -
                misBRST * consultaData[dias[index]].value,
              diario:
                (misBRST * consultaData[0].value -
                  misBRST * consultaData[dias[index]].value) /
                dias[index],
    ...
    contrato.BRST_TRX_Proxy.MIN_DEPOSIT()
      .call()
      .then((minCompra) => {
        this.setState({ minCompra: utils.normalizarNumero(minCompra) });
      });

    contrato.BRST.allowance(accountAddress, contrato.BRST_TRX_Proxy.address)
      .call()
      .then((depositoBRUT) => {
    ...
    let deposito =
      await contrato.BRST_TRX_Proxy.todasSolicitudes(accountAddress).call();
    let myids = [];
    ...
    let deposits =
      await contrato.BRST_TRX_Proxy.solicitudesPendientesGlobales().call();
    deposits = deposits[0];
    ...
    let adminsBrst = [
      "TWVVi4x2QNhRJyhqa7qrwM4aSXnXoUDDwY",
      "TWqsREyZUtPkBNrzSSCZ9tbzP3U5YUxppf",
      "TB7RTxBPY4eMvKjceXj8SWjVnZCrWr4XvF",
    ];

    let balance_Pool = new BigNumber(
      await tronWeb.trx.getBalance(contrato.BRST_TRX_Proxy.address),
    ).shiftedBy(-6);
    ...
    let isOwner =
      accountAddress ===
      tronWeb.address.fromHex(await contrato.BRST_TRX_Proxy.owner().call());
    let isAdmin = false;
    ...
    for (let index = 0; index < deposits.length; index++) {
      let pen = await contrato.BRST_TRX_Proxy.verSolicitudPendiente(
        deposits[index],
      ).call();
      pen = pen[0];
      ...
      let cantidadTrx = new BigNumber(parseInt(pen.brst))
        .times(parseInt(pen.precio))
        .shiftedBy(-6);
      ...
      if (myids.includes(parseInt(deposits[index])) && diasrestantes > 0) {
        boton = (
          <button
            className="btn btn-info ms-4 disabled"
            disabled
            aria-disabled="true"
          >
            {"Processing Unfreeze TRX "}{" "}
      ...
      if (
        (myids.includes(parseInt(deposits[index])) && diasrestantes <= 0) ||
        isOwner
      ) {
        if (
          balance_Pool.toNumber() < cantidadTrx.shiftedBy(-6).dp(6).toNumber()
        ) {
          boton = (
            <button
              className="btn btn-info ms-4 disabled"
      ...
      if (myids.includes(parseInt(deposits[index])) || isOwner || isAdmin) {
            ...
            <>
              ...
              <a
                ...
                href={
                  ...
                  tronWeb.address.fromHex(pen.wallet)
                }
              >
                {tronWeb.address.fromHex(pen.wallet)}
              </a>
        ...
        globDepositos[index] = (
          <div
            className="row mt-4"
            id={"sale-" + parseInt(deposits[index])}
            key={"glob" + parseInt(deposits[index])}
          >
            <div className="col-12 mb-2">
              <h4 className="fs-20 text-black">
                {t("brst.sale", { number: parseInt(deposits[index]) })} {" -> "}
                {parseInt(pen.brst) / 10 ** 6} BRST
              </h4>
            </div>
            <div className="col-sm-6 mb-2">
              <p className="mb-0 fs-14">
                {extraData}
                <b>Total: </b>
                {cantidadTrx.shiftedBy(-6).dp(6).toString(10)} TRX<br></br>
                <b>{t("brst.price")}</b> {parseInt(pen.precio) / 10 ** 6} TRX
                <br></br>
            ...
            <div className="col-sm-6 mb-2">
              <p className="mb-0 fs-14">
                {t("brst.unStaking", { days: diasrestantes })}
                {cantidadTrx.shiftedBy(-6).dp(6).toString(10)} TRX
              </p>
              <br></br>
              {boton}
            </div>
    ...
    let ownerPanel = (
      <>
        <input
          type="text"
          id="wallet"
          placeholder="wallet to white list"
        ></input>
        <button
          ...
          onClick={async () => {
            let inputs = [
              {
                type: "address",
                value: tronWeb.address.toHex(document.getElementById("wallet")),
              },
            ...
            try {
              let trigger =
                await tronWeb.transactionBuilder.triggerSmartContract(
                  tronWeb.address.toHex(contrato.BRST_TRX_Proxy_fast.address),
                  funcion,
                  {},
                  inputs,
        ...
        <br></br>
        TRON_RR:{" "}
        {utils.normalizarNumero(await contrato.BRST_TRX_Proxy.TRON_RR().call())}
        <br></br>
      </>
    );
    ...
    if (isAdmin || isOwner) {
      globDepositos.push(
        <div key="admin-panel">{isOwner ? ownerPanel : <></>}</div>,
      );
    }

    globDepositos.push(
      <div key="balnce-panel">
    ...
    energyOn = await fetch(config.BOT_URL)
      .then((r) => r.json())
      .then((r) => r.available)
      .catch(() => false);
  ...
  async preClaim(id) {
    ...
    let inputs = [{ type: "uint256", value: id }];
    let funcion = "retirar(uint256)";
    const options = {};
    var transaccion = await tronWeb.transactionBuilder
      .triggerConstantContract(
        tronWeb.address.toHex(contrato.BRST_TRX_Proxy.address),
        funcion,
        options,
        inputs,
        tronWeb.address.toHex(accountAddress),
      ...
      this.setState({
        ...
        ModalBody: (
          <>
            {textoModal[1]}{" "}
            <b>
              {eenergy} {textoModal[2]}
            </b>
            {textoModal[3]}
            <b>
              {userEnergy} {textoModal[2]}
            </b>{" "}
            {textoModal[4]}{" "}
            <b>
              {requerido} {textoModal[2]}
            </b>
            {textoModal[5]}
            <b>{precio.toString(10)} TRX</b>
            {textoModal[6]}
            <br></br>
            <br></br>
            <button
                  ...
                  transaction = await tronWeb.trx
                    ...
                    .then(() => {
                      this.setState({
                        ...
                        ModalBody: (
                          <>
                            Retiro is Done {transaction.txid}
                            <br></br>
                            <br></br>
                            <button
                              type="button"
  ...
  async calculoEnergy(rapida = false) {
    ...
    if (token) {
      ...
      let contract_base_token = tronWeb.contract(utils.abi_TOKEN, token);
      let decimals_base = parseInt(await contract_base_token.decimals().call());

      let aprove = await contract_base_token
        .allowance(accountAddress, sunswapRouter)
        .call();
      ...
      if (aprove <= 1 * 1e6) {
        ...
        const options = {};
        let trigger = await tronWeb.transactionBuilder
          .triggerConstantContract(
      ...
      let inputs = [
        ...
        {
          type: "uint256[]",
          value: distributeTokens(
            consulta.tokens.length,
            consulta.poolVersions,
          ),
        },
        { type: "uint24[]", value: consulta.poolFees },
        {
          ...
          value: [
            new BigNumber(consulta.amountIn)
              .shiftedBy(decimals_base)
              .dp(0)
              .toString(10),
            new BigNumber(consulta.amountOut)
              .times(0.995)
              .shiftedBy(6)
              .dp(0)
              .toString(10),
      ...
      let trigger = await tronWeb.transactionBuilder
        .triggerConstantContract(
          tronWeb.address.toHex(sunswapRouter),
          funcion,
          options,
          inputs,
          tronWeb.address.toHex(accountAddress),
      ...
      if (trigger.energy_used) {
        ...
        energyRequired = energyRequired.plus(
          trigger.energy_used * energyFactor,
        );
      }
    }
    ...
    if (to === "brst") {
      ...
      let inputs = [{ type: "uint256", value: "1000000" }];
      ...
      let trigger = await tronWeb.transactionBuilder
        .triggerConstantContract(
          tronWeb.address.toHex(AddressContract),
          funcion,
          options,
          inputs,
          tronWeb.address.toHex(accountAddress),
    ...
    if (energyRequired.toNumber() <= 0) {
      ...
      if (energyRequired.toNumber() < 32000)
        energyRequired = new BigNumber(32000);
    }
  ...
  async rentEnergy(cantidad) {
    ...
    this.setState({
      ...
      ModalBody: (
        <>
          sign the following transaction if you agree to pay for the energy
          lease.
        </>
      ),
    });
    ...
    this.setState({
      ...
      ModalBody: (
        <>
          One of our robots is going to pick up your energy to deliver it to you
          as soon as possible, this will not take long please wait.
        </>
      ),
    });
    ...
    if (consulta2.result) {
      this.setState({
        ...
        ModalBody: (
          <>
            {t("brst.alert.done", { returnObjects: true })[1]}
            <br></br>
            <button
              type="button"
              ...
              className="btn btn-success"
            >
              {t("brst.alert.done", { returnObjects: true })[2]}
            </button>
          </>
        ),
      });
      ...
      this.setState({
        ...
        ModalBody: (
          <>
            ...
            <br></br>
            <br></br>
            you have <b>{userEnergy.toLocaleString("en-US")} energy</b>
            <br></br>
            Rent <b>
              {eenergy.toNumber().toLocaleString("en-US")} energy
            </b> for <b>{precio.toString(10)} TRX</b>
            <br></br>
            <br></br>
            <button
              type="button"
            ...
            >
              Proceed without renting energy{" "}
            </button>
          </>
        ),
      });
  ...
  async suawpTokenFromTRX(select = 0) {
    ...
    let token =
      select === 0
        ...
        : "TXDk8mbtRbXeYuMNS83CfKPaYYT8XWv9Hz"; //USDT : USDD

    let currentPrice = select === 0 ? precioUSDT : precioUSDD;

    let sunswapRouter = "TCFNp179Lg46D16zKoumd4Poa2WFFdtqYj"; // V3

    let contrato = tronWeb.contract(utils.abi_SUNSWAP, sunswapRouter);
    //console.log(contrato)

    let contract_base_token = tronWeb.contract(utils.abi_TOKEN, token);
    let decimals_base = parseInt(await contract_base_token.decimals().call());

    let balance_usdt = new BigNumber(
      parseInt(await contract_base_token.balanceOf(accountAddress).call()),
    ).shiftedBy(-decimals_base);
    ...
    if (
      balance_usdt.dp(2).toNumber() < 1 ||
      monto.toNumber() > balance_usdt.toNumber()
    ) {
      this.setState({
        ModalTitulo: "ALERT",
        ModalBody: (
          <>
            ...
            {await contract_base_token.name().call()} to complete this
            operation.
          </>
        ),
      });
    ...
    if (aprove <= balance_usdt.toNumber()) {
      ...
      const options = {};
      let trigger = await tronWeb.transactionBuilder.triggerSmartContract(
        tronWeb.address.toHex(contract_base_token.address),
        funcion,
        options,
        inputs,
        tronWeb.address.toHex(accountAddress),
      ...
      transaction = await window.tronLink.tronWeb.trx
        ...
        .catch((e) => {
          this.setState({
            ModalTitulo: t("brst.alert.nonEfective", {
              returnObjects: true,
            })[0],
            ModalBody:
              t("brst.alert.nonEfective", { returnObjects: true })[1] +
              " | " +
              e.toString(),
          });
    ...
    let inputs = [
      ...
      {
        type: "uint256[]",
        value: distributeTokens(consulta.tokens.length, consulta.poolVersions),
      },
      { type: "uint24[]", value: consulta.poolFees },
      {
        type: "(uint256,uint256,address,uint256)",
    ...
    const options = { feeLimit: 10000 * 1e6, callValue: 0 };
    let trigger = await tronWeb.transactionBuilder.triggerSmartContract(
      tronWeb.address.toHex(contrato.address),
      funcion,
      options,
      inputs,
      tronWeb.address.toHex(accountAddress),
    ...
    if (!transaction) return;
    transaction = await tronWeb.trx.sendRawTransaction(transaction).then(() => {
      this.setState({
  ...
  async compra(amount = new BigNumber(0)) {
    ...
    if (amount <= 0) {
      this.setState({
        ...
        ModalBody: (
          <>
            {t("brst.alert.errorInput", { returnObjects: true })[1]}
            <br></br>
            <br></br>
            <button
              type="button"
    ...
    if (balance >= amount) {
      if (amount >= minCompra) {
          ...
          .then((r) => {
            if (r.result) {
              this.setState({
                ...
                ModalBody: (
                  <>
                    {t("brst.alert.compra", { returnObjects: true })[1]}
                    <br></br>
                    <br></br>
                    <button
                      type="button"
        ...
        this.setState({
          ModalTitulo: t("error"),
          ModalBody: t("brst.alert.errGreater", {
            returnObjects: true,
            min: minCompra,
          })[0],
        });
      ...
      this.setState({
        ModalTitulo: t("error"),
        ModalBody: t("brst.alert.errFunds", {
          returnObjects: true,
          min: minCompra,
        })[0],
      });
  ...
  async venta(rapida) {
    ...
    let AddressContract = tronWeb.address.toHex(
      contrato.BRST_TRX_Proxy.address,
    );
    if (rapida) {
      AddressContract = tronWeb.address.toHex(
        contrato.BRST_TRX_Proxy_fast.address,
      );
    }

    let aprovado = await contrato.BRST.allowance(
      accountAddress,
      AddressContract,
    ).call();
    ...
    if (aprovado <= amount.toNumber()) {
      ...
      const options = {};
      let trigger = await tronWeb.transactionBuilder.triggerSmartContract(
        tronWeb.address.toHex(contrato.BRST.address),
        funcion,
        options,
        inputs,
        tronWeb.address.toHex(accountAddress),
      ...
      aprovado = await contrato.BRST.allowance(
        accountAddress,
        AddressContract,
      ).call();
      aprovado = parseInt(aprovado);
    ...
    if (aprovado >= amount.toNumber()) {
      if (amount.toNumber() >= minventa && true) {
        ...
        let inputs = [{ type: "uint256", value: amount.toString(10) }];
      ...
      if (amount > aprovado) {
        if (aprovado <= 0) {
          this.setState({
            ModalTitulo: "Info",
            ModalBody:
              "You do not have enough aproved funds in your account you place at least " +
              minventa +
              " BRST",
          });
          ...
          this.setState({
            ModalTitulo: "Info",
            ModalBody:
              "You must leave 21 TRX free in your account to make the transaction",
          });
  ...
  render() {
    ...
    let swapButton = (
      <button
        className="btn btn-success"
        style={{ width: "100%" }}
        onClick={() => this.preExchange(rapida)}
      >
    ...
    if (
      to === "trx" &&
      rapida &&
      retiroRapido.toNumber() < valueTo.toNumber()
    ) {
      swapButton = (
                  ...
                  >
                    <div className="card coin-content">
                      <div className="card-header border-0 flex-wrap">
                        <div className="mb-2">
                          <h4 className="heading m-0">
                            {t("tokenChart", { token: "BRST" })}
                          </h4>
                      ...
                      <div className="card-body">
                          ...
                          <div className="d-flex align-items-center justify-content-between flex-wrap">
                            ...
                            <div className="price-content">
                              ...
                              <h4 className="font-w600">
                                {(this.state.tokensEmitidos * 1).toFixed(2)}{" "}
                                BRST
                              </h4>
                            </div>
                            <div className="price-content">
                              ...
                              <h4 className="font-w600">
                                {(this.state.enBrutus * 1).toFixed(2)} TRX
                              </h4>
                            </div>
                          </div>
                        </div>
                        ...
                        <select
                          ...
                          value={this.state.cantidadDatos}
                          ...
                          <div className="media d-block">
                            <img
                              onClick={() => {
                                ...
                                this.setState({
                                  ModalTitulo: t("brst.alert.donate", {
                                    returnObjects: true,
                                  })[0],
                                  ModalBody: (
                                    <>
                                      ...
                                      <button
                                        ...
                                        onClick={async () => {
                                          ...
                                          donacion = new BigNumber(donacion)
                                            .shiftedBy(6)
                                            .dp(0);
                                          ...
                                          transaction =
                                            await window.tronLink.tronWeb.trx
                                              .sign(transaction)
                                              .catch((e) => {
                                                this.setState({
                                                  ModalTitulo: t(
                                                    "brst.alert.nonEfective",
                                                    { returnObjects: true },
                                                  )[0],
                                                  ModalBody:
                                                    t(
                            ...
                            <div className="media-content">
                              ...
                              <span className="my-4 fs-16 font-w600 d-block">
                                1 BRST = {this.state.precioBrst} TRX
                              </span>
                  ...
                  >
                    <div className="card quick-trade">
                      ...
                      <div className="container">
                        <div className="row">
                          ...
                          <div className="col-2 ">
                            <img
                              height="42px"
                              src={"/images/token/" + this.state.from + ".png"}
                              alt="tron logo"
                            />
                          </div>
                          <div
                          ...
                          <div className="col-7" style={{ paddingLeft: "0px" }}>
                            <input
                              ...
                              onInput={() =>
                                this.calcExchange(
                                  false,
                                  this.state.from + "_" + this.state.to,
                                )
                              }
                              placeholder={minCompra}
                              min={this.state.minCompra}
                              step={0.1}
                            ></input>
                          </div>
                        </div>
                        ...
                        <div className="row">
                          ...
                          <div className="col-2">
                            <img
                              height="42px"
                              src={"/images/token/" + this.state.to + ".png"}
                              alt="brst logo"
                            />
                          </div>
                          <div
                          ...
                          <div className="col-7" style={{ paddingLeft: "0px" }}>
                            <input
                              ...
                              onInput={() =>
                                this.calcExchange(
                                  true,
                                  this.state.to + "_" + this.state.from,
                                )
                              }
                              placeholder={minventa}
                              min={this.state.minventa}
                              step={0.0001}
                            ></input>
                          </div>
                        </div>
                      ...
                      <div className="card-footer border-0">
                        {retiradas}
                        <div className="row">
                          <div className="col-12 text-center">{swapButton}</div>
                        </div>
                        <div
                        ...
                        >
                          <p className="mb-0 fs-14">
                            {t("brst.energy", {
                              e1: userEnergy.toLocaleString("en-US"),
                            })}
                          </p>
                          <p className="mb-0 fs-14">
                            Fee ~ {this.state.useTrx} TRX
                          </p>
                        </div>
                      </div>
                    </div>
                          ...
                          <table className="table text-center bg-warning-hover order-tbl">
                            ...
                            <tbody>
                              ...
                              >
                                ...
                                <td>
                                  {this.state.balanceUSDT.toLocaleString(
                                    "en-US",
                                  )}
                                </td>
                                <td>--.--</td>
                              ...
                              >
                                ...
                                <td>
                                  {this.state.balanceUSDD.toLocaleString(
                                    "en-US",
                                  )}
                                </td>
                                <td>--.--</td>
            ...
            <div className="card">
              <div className="card-header">
                ...
                <h6 className="card-subtitle">
                  {this.state.misBRST} BRST ={" "}
                  {(this.state.misBRST * this.state.precioBrst).toFixed(3)} TRX
                </h6>
              </div>
              <div className="card-body">
                <div className="table-responsive">
                      ...
                      {earnings.map((objeto) => (
                        <tr key={objeto.dias.toString()}>
                          ...
                          <td className="color-primary">
                            {objeto.diario.toFixed(6)} TRX/{t("day")}{" "}
                          </td>
                        </tr>
                      ))}
                    </tbody>
                ...
                <div>
                  <h4 className="fs-20 text-black">
                    ...
                    <button
                      ...
                      onClick={() => this.estado()}
                    >
                      {t("brst.request", { returnObjects: true })[1]}{" "}
                      <i className="bi bi-arrow-repeat"></i>
                    </button>
                  </h4>
                  <p className="mb-0 fs-12">
                    {t("brst.request", { returnObjects: true })[2]}
                  </p>
                </div>
              </div>
              <div className="card-body">{globDepositos}</div>
            </div>
          </div>
            ...
            <div className="card">
                ...
                <h6
                  ...
                  onClick={() => {
                    document.getElementById("hold").value =
                      this.state.balanceBRST;
                    this.handleChangeCalc({
                      target: { value: this.state.balanceBRST },
                    });
                  }}
                >
                  {t("brst.mystaking")}
              ...
              <div className="card-body">
                ...
                <input
                  type="number"
                  id="daysProm"
                  defaultValue={tiempoPromediado}
                  placeholder={tiempoPromediado + " days"}
                  min={1}
                  step={1}
                  onInput={async () => {
                    let daysProm = parseInt(
                      document.getElementById("daysProm").value,
                    );
                    await this.setState({
                      tiempoPromediado: isNaN(daysProm) ? 1 : daysProm,
                    });
                    this.estado();
                  }}
                ></input>
                ...
                <div
                  ...
                  style={{
                    marginTop: "30px",
                    height: "420px",
                    border: "2px solid rgba(207, 207, 207, 0.97)",
                    borderRadius: "10px",
                  }}
                >
                  <table className="table table-hover table-responsive-sm">
                      ...
                      <tr>
                        <th>
                          ...
                          <input
                            type="number"
                            id="days"
                            defaultValue={diasCalc}
                            onInput={this.handleChangeDias}
                          ></input>
                          <br></br>
                          <br></br>
                          <button
                      ...
                      {days.map((obj) => {
                        ...
                        return (
                          ...
                          >
                            <th>{obj.days}</th>
                            <td>
                              {obj.amount} BRST<br></br>
                              {(obj.amount * precioBrst).toFixed(3)} TRX
                            </td>
                            <td>
                              {(
                                obj.amount *
                                precioBrst *
                                ((crecimientoPorcentual * obj.days) / 100)
                              ).toFixed(6)}{" "}
                              TRX
                            </td>
                          </tr>
              ...
              <div className="card-body">
                <p>
                  ...
                  <a
                    ...
                    rel="noopener noreferrer"
                    href={
                      "https://tronscan.org/#/contract/" + config.BRST + "/code"
                    }
                  >
                    {config.BRST}
                  </a>
                  <br></br>
                  <b>Regular withdrawals:</b>{" "}
                  <a
                    target="_blank"
                    rel="noopener noreferrer"
                    href={
...
const StakingComponent = withTranslation()(Staking);
