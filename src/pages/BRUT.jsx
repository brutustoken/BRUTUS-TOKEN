...
const BrutComponent = (props) => {
    ...
    if (aprovado <= 0) {
      ...
      await tronWeb.trx.sendRawTransaction(transaction)
        .then((r) => {
          ...
          setStatePartial({ msj: { title: "Result", message: <>Transacction hash: {r.txid}</> } })
  ...
  const venta = async () => {
    ...
    if (amount < minventa) {
      ...
      setStatePartial({ msj: { title: "Error", message: "Place an amount greater than " + minventa + " BRUT" } })
      return;
    }
  ...
  const consultarPrecio = async () => {
    ...
    let precio;
    try {
      response = await fetch(apiUrl).then((res) => { return res.json() }).catch(error => { console.error(error) })
    ...
    try {
      response = await fetch(config.market_brut).then((res) => { return res.json() }).catch(error => { console.error(error) })
  ...
  */
  ...
  let { minCompra, minventa, msj, totalCirculando, precioBRUT, cambio24h, enBrutus, depositoBRUT, depositoUSDT } = state;
              ...
              <div className="col-xl-9 col-xxl-9 wow fadeInLeft" data-wow-delay="0.2s">
                <div className="card coin-content">
                  ...
                  <div className="card-body">
                    ...
                    <select className="btn-secondary style-1 default-select" style={{ backgroundColor: "white" }} value={state.cantidadDatos} onChange={handleChange2}>
                      {options2.map((option) => (
                        <option key={option.label.toString()} value={option.value}>{option.label}</option>
                      ))}
                    </select>
                    {" | "}
                    <select className="btn-secondary style-1 default-select" style={{ backgroundColor: "white" }} value={state.temporalidad} onChange={handleChange}>
                      {options.map((option) => (
                        <option key={option.label.toString()} value={option.value}>{option.label}</option>
                      ))}
                    </select>
                    ...
                    <div className="text-center">
                      <div className="media d-block">
                        ...
                        <div className="media-content">
                          ...
                          <span className="my-4 fs-16 font-w600 d-block">1 BRUT = {state.precioBRUT} USD</span>
              ...
              <div className="col-xl-6 col-sm-6 wow fadeInUp" data-wow-delay="0.4s">
                <div className="card quick-trade">
                  ...
                  <div className="card-body pb-0">
                    ...
                    <div className="row">
                      <div className="col-6">
                        <input className="form-control form-control text-end" type="number" id="amountBRUT" onChange={handleChangeBRUT} placeholder={minventa} min={state.minventa} max={state.balanceBRUT} value={state.valueBRUT} step={0.5} ></input>

                      </div>
                      <div className="col-6">
                        <input className="form-control form-control text-end" type="number" id="amountUSDT" onChange={handleChangeUSDT} placeholder={minCompra} min={state.minCompra} max={state.balanceUSDT} value={state.valueUSDT} ></input>
                      </div>
                    </div>
                  </div>
                  <div className="card-footer border-0">
                    <div className="row">
                      <div className="col-6">
                        <button className="btn d-flex  btn-success justify-content-between w-100" onClick={() => compra()}>
                          {depositoUSDT}
                      ...
                      <div className="col-6">
                        <button className="btn d-flex  btn-danger justify-content-between w-100" onClick={() => venta()}>
                          {depositoBRUT}
                  ...
                  <div className="card-body p-3 py-0">
                    <div className="table-responsive">
                      <table className="table text-center bg-warning-hover order-tbl">
                        ...
                        <tbody>
                          <tr style={{ cursor: "pointer" }} onClick={() => { handleChangeBRUT({ target: { value: state.balanceBRUT } }) }}>
                            <td className="text-left">BRUT</td>
                            <td>{state.balanceBRUT}</td>
                          </tr>
                          <tr style={{ cursor: "pointer" }} onClick={() => { handleChangeUSDT({ target: { value: state.balanceUSDT } }) }}>
