...
class Inicio extends Component {
  ...
  async estado() {
    ...
    let precioBrst = utils.normalizarNumero(
    ...
    contrato.BRST.balanceOf(accountAddress)
      .call()
      .then((result) => {
        this.setState({ misBRST: utils.normalizarNumero(result) });
      })
      .catch(console.error);

    contrato.BRUT.balanceOf(accountAddress)
      .call()
      .then((result) => {
        this.setState({ misBRUT: utils.normalizarNumero(result) });
      })
      .catch(console.error);

    if (accountAddress !== "T9yD14Nj9j7xAB4dbGeiX9h8unkKHxuWwb") {
      contrato.BRGY.balanceOf(accountAddress)
        .call()
        .then((result) => {
          this.setState({ misBRGY: utils.normalizarNumero(result, 0) });
        })
        .catch(console.error);

      contrato.BRLT.balanceOf(accountAddress)
        .call()
        .then((result) => {
          this.setState({ misBRLT: utils.normalizarNumero(result, 0) });
        })
        .catch(console.error);
    }
  }
                  ...
                  >
                    <div className="table-responsive dataTablemarket">
                      <table
                        id="example"
                        style={tableStyle}
                        className="table shadow-hover display"
                      >
                        <thead>
                          <tr>
                            <th>{t("inicio.name")}</th>
                        ...
                        <tbody>
                          <tr>
                              ...
                              >
                                ...
                                <div style={{ paddingLeft: "10px" }}>
                                  <span className="text-muted ms-2">
                                    ...
                                    Brutus Token<br></br>
                                    {this.state.precioBrut} USDT{" "}
                                    {this.subeobaja(this.state.varBrut)}
                                    <span
                                      className={
                                        "text-" +
                                        this.textoE(this.state.varBrut)
                                      }
                                    >
                                      {this.state.varBrut.toFixed(3)}%
                                    </span>
                                  </span>
                                </div>
                              </a>
                            ...
                            <td style={cellStyle}>
                              {(
                                this.state.misBRUT * this.state.precioBrut
                              ).toFixed(3)}{" "}
                              USDT
                            </td>
                          ...
                          <tr>
                              ...
                              >
                                ...
                                <div style={{ paddingLeft: "10px" }}>
                                  <span className="text-muted ms-2">
                                    ...
                                    Brutus Tron Staking<br></br>
                                    {this.state.precioBrst.toFixed(6)} TRX{" "}
                                    {this.subeobaja(this.state.varBrst)}
                                    <span
                                      className={
                                        "text-" +
                                        this.textoE(this.state.varBrst)
                                      }
                                    >
                                      {this.state.varBrst.toFixed(3)}%
                                    </span>
                                  </span>
                                </div>
                              </a>
                            ...
                            <td style={cellStyle}>
                              {this.state.misBRST.toLocaleString("en-US")}
                            </td>
                            <td style={cellStyle}>
                              {(
                                this.state.misBRST * this.state.precioBrst
                              ).toLocaleString("en-US")}{" "}
                              TRX
                            </td>
                            <td style={lastCellStyle}>
                              {(
                                this.state.misBRST * this.state.precioBrstUSD
                              ).toLocaleString("en-US")}{" "}
                              USD
                            </td>
                          </tr>
                          ...
                          <tr>
                            ...
                            <td style={{ ...cellStyle, borderBottom: "none" }}>
                              {this.state.misBRLT * 100} TRX
                            </td>
                            <td
                              style={{ ...lastCellStyle, borderBottom: "none" }}
                            >
                              {(
                                this.state.misBRLT *
                                100 *
                                this.state.precioTRX
                              ).toFixed(1)}{" "}
                              USD
                            </td>
                          </tr>
...
const InicioWT = withTranslation()(Inicio);

export default InicioWT;
